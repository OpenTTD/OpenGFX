...
function list_objectives($orderby = 'file', $direction = 'ASC') {
	...
	echo '<h2>Targets for 0.1.0-alpha5</h2>';

	//get resultset
	$result = mysql_query("SELECT * FROM `".$cfg_db['table']."` WHERE (`done`=0 AND (
	(`file`='ogfx1_base' AND ((`range_start`>1967 AND `range_end`<4574) OR (`range_start`>4587 AND `range_end`<4627))) 
	...
	//Targets for 0.1.0-alpha6
	echo '<h2>Targets for 0.1.0-alpha6</h2>';

	//get resultset
	$result = mysql_query("SELECT * FROM `".$cfg_db['table']."` WHERE (`done`=0 AND `file`='ogfx1_base' AND (`range_start`>4492 AND `range_end`<4604)) ORDER BY `".$orderby."` ".$direction." , `range_start`, `id`;", $cfg_db['server_link']);
	...
	//get resultset
	$result = mysql_query("SELECT * FROM `".$cfg_db['table']."` WHERE (`done`=0 AND (
	(`file`='ogfx1_base' AND (`range_end`<2981 OR `range_start`>4625))
